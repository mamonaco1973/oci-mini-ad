...
ECDSA P-256 key pair generated by `tls_private_key` in `01-directory/accounts.tf`. Written to `01-directory/keys/Private_Key` (0600) and `01-directory/keys/Private_Key.pub`. Gitignored — never committed. Used for both DC access (via bastion tunnel) and Linux client direct SSH.
